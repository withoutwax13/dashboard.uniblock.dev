...
describe("Scenario: Verify invite collaborator feature	", () => {
    ...
    it("TC014 - Verify deletion of collaborator by an admin", () => {
        ...
        //Verify that the modal contains a close button and a delete button does not exist.
        cy.get('div').contains('Remove user from project').should('not.exist')

        //Verify that the collaborator is no longer present in the collaborator list.
        cy.get("tbody").within(() => {
          cy.get('tr').find('td').eq(0).should('not.contain', 'testEmail@gmail.com');
        });	
    })
})